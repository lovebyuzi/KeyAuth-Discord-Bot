...
async def on_ready():      
	...
	print(f"Bot Developer: Zempex#9492")
	bot.launch_time = datetime.utcnow()
	await bot.change_presence(status=discord.Status.online,activity=discord.Game(f"Prefix: {prefix} | Developer: Zempex#9492"))
...
async def claim(ctx,lic):
	...
	try:
		...
		req = requests.get(f"https://keyauth.win/api/seller/?sellerkey=REPLACEKEY&type=activate&user={username}&key={lic}&pass={password}")
...
async def resethwid(ctx,user,reason):
	...
	if role not in ctx.author.roles:
		return
	req = requests.get(f"https://keyauth.win/api/seller/?sellerkey=REPLACEKEY&type=resetuser&user={user}")
...
async def deleteuser(ctx,user):
	req = requests.get(f"https://keyauth.win/api/seller/?sellerkey=REPLACEKEY&type=deluser&user={user}")
	if req.json()["success"] == True:
...
async def banaccount(ctx,user,*,reason=None):
	req = requests.get(f"https://keyauth.win/api/seller/?sellerkey=REPLACEKEY&type=banuser&user={user}&reason={reason}")
	if req.json()["success"] == True:
...
async def unbanaccount(ctx,user):
	req = requests.get(f"https://keyauth.win/api/seller/?sellerkey=REPLACEKEY&type=unbanuser&user={user}")
	if req.json()["success"] == True:
		embed=discord.Embed(title="**Moderation: User was unbanned**",description=f"{user}, Has been unbanned from Zempex Server.\nReason is `{reason}`. This Unban was requested from\n{ctx.author.mention}",colour=0x42F56C)
		embed.add_field(name="UnBanned By:", value=f"``{ctx.author}``", inline=True)
		await ctx.send(embed=embed)
...
async def deletelicense(ctx,lic):
	req = requests.get(f"https://keyauth.win/api/seller/?sellerkey=REPLACEKEY&type=del&key={lic}")
	if req.json()["success"] == True:
...
async def extend(ctx,user,sub,days):
	req = requests.get(f"https://keyauth.win/api/seller/?sellerkey=REPLACEKEY&type=extend&user={user}&sub={sub}&expiry={days}")
	print(req.text)
	if req.json()["success"] == True:
		await ctx.send(embed=discord.Embed(title="Username Extended!",description='Succesfully Extended User',colour=0x42F56C))
	else:
...
async def blacklist(ctx,inputofuser):
	...
	req = requests.get(f"https://keyauth.win/api/seller/?sellerkey=REPLACEKEY&type=black&{oii}")
	if req.json()["success"] == True:
...
async def unblacklist(ctx,inputofuser):
	...
	if len(inputofuser) == 44:
		...
		oii = f"ip={inputofuser}"
	req = requests.get(f"https://keyauth.win/api/seller/?sellerkey=REPLACEKEY&type=delblack&{oii}")
	if req.json()["success"] == True:
...
async def gen(ctx,day:int):
	...
	if role not in ctx.author.roles:
		return
	req = requests.get(f"https://keyauth.win/api/seller/?sellerkey=REPLACEKEY&type=add&expiry={day}&mask=XXXXXX-XXXXXX-XXXXXX-XXXXXX-XXXXXX-XXXXXX&level=1&amount=1&format=json")
	if req.json()["success"] == True:
		if day == 1:
			sub = "daily"
		elif day == 7:
...
async def accountinfo(ctx,user):
	req = requests.get(f"https://keyauth.win/api/seller/?sellerkey=REPLACEKEY&type=userdata&user={user}")
	if req.json()["success"] == True:
